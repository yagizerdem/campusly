...
export default function FeedPost({ post }: Props) {
  ...
  return (
    <article className="w-full rounded-lg border border-border bg-card p-5 text-card-foreground transition-shadow duration-300 hover:shadow-[0_4px_12px_rgba(23,33,43,0.05)]">
      ...
      {images.length > 0 && (
        <PostImageGallery images={images} title={post.postTitle} />
      )}
...
function PostImageGallery({
  ...
  if (images.length === 2) {
    return (
      <div className="grid h-[300px] grid-cols-2 gap-2 overflow-hidden rounded-lg">
    ...
    <div className="grid h-[300px] grid-cols-2 gap-2 overflow-hidden rounded-lg">
      ...
      <div className="grid min-h-0 grid-rows-2 gap-2">
        {visibleImages.slice(1).map((image, index) => {
          const hiddenImageCount = images.length - 3;
          const showImageCount = index === 1 && hiddenImageCount > 0;
          ...
          return (
            ...
            >
              ...
              />
              {showImageCount && (
                <div className="absolute inset-0 grid place-items-center bg-black/50 text-2xl font-semibold text-white">
                  +{hiddenImageCount}
                </div>
              )}
            </div>

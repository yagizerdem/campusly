...
} from "@packages/shared/dto/tag-dto.js";
...
export async function assignTag(profileId: string, tagDto: AssignTagDto) {
  ...
  if (club.clubAdminId !== profileId) {
    throw AppError.from({
      machineCode: ErrorMachineCode.INSUFFICIENT_PERMISSIONS,
      message: "You are not authorized to assign tags to this club.",
...
export async function removeAssignedTag(
  ...
  if (club.clubAdminId !== profileId) {
    throw AppError.from({
      machineCode: ErrorMachineCode.INSUFFICIENT_PERMISSIONS,
      message: "You are not authorized to remove tags from this club.",
      statusCode: HttpStatusCode.FORBIDDEN,
      isOperational: true,
    });
  }
